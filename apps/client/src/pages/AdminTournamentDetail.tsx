...
export default function AdminTournamentDetail() {
    ...
    try {
      ...
      for (let i = 0; i < tt.rounds.length; i++) {
        ...
        for (const mid of mids) {
          try {
            const mres = await adminApi.get(`/matches/${mid}`)
            const m = mres.data.match
            const stats = mres.data.stats
            const [a, b] = await Promise.all([
              adminApi.get(`/posts/${m.postAId}`),
              adminApi.get(`/posts/${m.postBId}`),
            ])
  ...
  useEffect(() => {
    function compute() {
      ...
      // For each round except last, map match index j -> next round index Math.floor(j/2)
  ...
  async function progress() {
    ...
    try {
      setProgressing(true)
      await adminApi.post(`/tournaments/${id}/progress`)
      await load()
    } catch (e: any) {
      alert(e?.response?.data?.error || 'İlerletilemedi')
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {t.status !== 'finished' && (
            <button onClick={progress} disabled={progressing} className="px-3 py-2 rounded bg-neutral-900 text-white disabled:opacity-60">{progressing ? 'İlerletiliyor...' : 'Turu İlerle'}</button>
          )}
          <button onClick={async ()=>{ if(!id) return; if(!confirm('Bu turnuvanın tüm oylarını sıfırlamak istiyor musunuz?')) return; await adminApi.post(`/votes/reset/tournament/${id}`); alert('Turnuva oyları sıfırlandı'); }} className="px-3 py-2 rounded border">Oyları Sıfırla</button>
        </div>
      </div>
